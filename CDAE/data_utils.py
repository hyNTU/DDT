...
def load_all(dataset, data_path):
	...
	train_data_pos = {}
	for x in train_data_list:
		if x[0] in train_data_pos:
			train_data_pos[x[0]].append(x[1])
		else:
			train_data_pos[x[0]] = [x[1]]
	...
	return train_data_list, valid_data_list, train_data_pos, valid_data_pos, test_data_pos, user_pos, user_num, item_num, train_mat, valid_mat, train_data_noisy
...
class DenseMatrixUsers(data.Dataset):
	...
	def change_data(self, user_index, dataset):
		self.dataset = dataset
		self.user_index = user_index
		assert len(self.dataset) == len(self.user_index)
