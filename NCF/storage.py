...
########################### Eval #####################################
def eval(model, valid_pos, mat, best_recall, count, epoch_loss):
    top_k = args.top_k
    model.eval()
    predictedIndices = []
    GroundTruth = []
    ...
    for users_valid in toolz.partition_all(15,users_in_valid):
        ...
        users_valid_torch = torch.tensor(users_valid).repeat_interleave(item_num).cuda()   # repeats users_valid for item_num times: 0,0..,1,1..1024,1024..
    ...
    if epoch_recall > best_recall:
        ...
        count = 0
        torch.save(epoch_loss, f'../data/overall_loss_{args.model}_{args.dataset}.pth')
...
for epoch in range(1000):
    ...
    if epoch >= args.epoch_eval:
        best_recall, count = eval(model_test, valid_pos, train_mat_dense, best_recall, count, epoch_loss)
    model_test.train()
    if count == 10:
