...
def load_all(dataset, data_path):
	...
	train_valid_mat = sp.dok_matrix((user_num, item_num), dtype=np.float32)
	train_data_list = []
	train_data_noisy = []
	for x in train_data:
	...
	return train_data_list, valid_data_list, test_data_pos, valid_pos, user_pos, user_num, item_num, train_mat, train_valid_mat, train_data_noisy
...
class NCFData(data.Dataset):
	def __init__(self, features,
		...
		self.groups = [0 for _ in range(2*len(features))]

	def ng_sample(self, user_dict=None):
		...
		if user_dict is not None:
			self.groups_fill = [user_dict[self.features_fill[idx][0]] for idx in range(len(self.features_fill))]
		else:
			self.groups_fill = self.groups

	def change_data(self, features, train_mat, noisy_or_not, is_training=0):
		self.features_ps = features
		self.train_mat = train_mat
		self.labels = [0 for _ in range(len(features))]
		self.groups = [0 for _ in range(2*len(features))]
	...
	def __getitem__(self, idx):
		...
		groups = self.groups_fill if self.is_training != 2 \
				else self.group
		...
		return user, item, label, noisy_label, group
